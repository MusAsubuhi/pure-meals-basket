...
class OrderCalculatorTest extends TestCase
{
    ...
    public function zero_subtotal_and_zero_discount_gives_zero_total(): void
    {
        ...
        $this->assertSame(0.0, $result->paymentRequired);
        $this->assertSame(0.0, $result->amountPaid);
        $this->assertSame(0.0, $result->balanceDue);
    }
    ...
    /** @test */
    public function payment_required_and_balance_due_are_calculated(): void
    {
        $result = $this->calculator->calculate(100000, 0, 30000);

        $this->assertSame(100000.0, $result->total);
        $this->assertSame(30000.0, $result->paymentRequired);
        $this->assertSame(30000.0, $result->amountPaid);
        $this->assertSame(70000.0, $result->balanceDue);
    }

    /** @test */
    public function full_payment_zeroes_balance(): void
    {
        $result = $this->calculator->calculate(100000, 0, 100000);

        $this->assertSame(100000.0, $result->total);
        $this->assertSame(100000.0, $result->paymentRequired);
        $this->assertSame(0.0, $result->balanceDue);
    }
    ...
    /** @test */
    public function negative_payment_required_throws_exception(): void
    {
        $this->expectException(InvalidArgumentException::class);
        $this->calculator->calculate(100000, 0, -1000);
    }
}
